    ...
    stages {
        ...
        stage('Test') {
            steps {
                ...
                script {
                    def linuxPath = sh(script: 'cygpath -u "${WORKSPACE}"', returnStdout: true).trim()
                    sh "docker run --rm -v ${linuxPath}:/workspace -w /workspace garage-app pytest tests/"
                }
            }
        }
        ...
        stage('Code Quality') {
            steps {
                ...
                script {
                    def linuxPath = sh(script: 'cygpath -u "${WORKSPACE}"', returnStdout: true).trim()
                    sh "docker run --rm -v ${linuxPath}:/workspace -w /workspace garage-app pylint garage.py || true"
                }
            }
        }
        ...
        stage('Security Scan') {
            steps {
                ...
                script {
                    def linuxPath = sh(script: 'cygpath -u "${WORKSPACE}"', returnStdout: true).trim()
                    sh "docker run --rm -v ${linuxPath}:/workspace -w /workspace garage-app bandit -r garage.py || true"
                }
            }
        }
